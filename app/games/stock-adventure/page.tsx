...
const STORAGE_KEY = "stocktoons_stock_adventure_progress"
const API_ENDPOINT = "/api/stock-adventure/progress"

export default function StockAdventureGamePage() {
  ...
  const loadProgressData = async () => {
    console.log("[Stock Adventure] Loading game progress...")
    try {
      // Load from localStorage first
      const localProgressData = localStorage.getItem(STORAGE_KEY)
      if (localProgressData) {
        const parsedProgress = JSON.parse(localProgressData)
        console.log("[Stock Adventure] Local progress loaded:", parsedProgress)
        setProgress(parsedProgress)
        setCurrentLevel(parsedProgress.currentLevel)
        setCurrentQuestion(parsedProgress.currentQuestion)

        // Check if game is completed
        if (parsedProgress.completedLevels.length === gameData.length) {
          setGameCompleted(true)
          console.log("[Stock Adventure] Game already completed!")
        }
      } else {
        console.log("[Stock Adventure] No local progress found, starting fresh")
      }

      // Try to sync with server
      try {
        console.log("[Stock Adventure] Attempting to sync with server...")
        const response = await fetch(API_ENDPOINT)
        if (response.ok) {
          const serverProgress = await response.json()
          console.log("[Stock Adventure] Server progress received:", serverProgress)
          if (
            serverProgress &&
            serverProgress.lastPlayed > (localProgressData ? JSON.parse(localProgressData).lastPlayed : "")
          ) {
            console.log("[Stock Adventure] Server progress is newer, using server data")
            ...
            console.log("[Stock Adventure] Local progress is newer or equal, keeping local data")
          }
        } else {
          console.warn("[Stock Adventure] Server sync failed with status:", response.status)
        }
      } catch (serverError) {
        console.warn("[Stock Adventure] Server sync error, using local progress:", serverError)
      }
    } catch (error) {
      console.error("[Stock Adventure] Failed to load progress:", error)
    } finally {
      setLoading(false)
      console.log("[Stock Adventure] Progress loading complete")
    }
  }
  ...
  const saveProgressData = async () => {
    try {
      ...
      console.log("[Stock Adventure] Progress saved to localStorage:", progress)
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log("[Stock Adventure] Progress saved to server:", result)
        } else {
          console.warn("[Stock Adventure] Server save failed with status:", response.status)
        }
      } catch (serverError) {
        console.warn("[Stock Adventure] Server save error, progress saved locally only:", serverError)
      }
    } catch (error) {
      console.error("[Stock Adventure] Failed to save progress:", error)
    }
  }

  const handleAnswerSelect = (answerIndex: number) => {
    if (showResult) return
    console.log(`[Stock Adventure] Answer selected: ${answerIndex}`)
  ...
  const handleSubmitAnswer = () => {
    ...
    console.log(
      `[Stock Adventure] Answer submitted - Correct: ${correct}, Selected: ${selectedAnswer}, Correct Answer: ${currentQ.correctAnswer}`,
    )
    ...
    if (correct) {
      ...
      console.log("[Stock Adventure] Correct answer! Score updated:", newProgress.totalScore)
      ...
      console.log("[Stock Adventure] Incorrect answer, showing hint option")
    }
  }
  ...
  const handleNextQuestion = () => {
    ...
    console.log(`[Stock Adventure] Moving to next - Last Question: ${isLastQuestion}, Last Level: ${isLastLevel}`)
    ...
    if (isLastQuestion) {
      if (isLastLevel) {
        ...
        console.log("[Stock Adventure] 🎉 GAME COMPLETED! Final Progress:", newProgress)
        setProgress(newProgress)
        setGameCompleted(true)
      } else {
        // Move to next level
        ...
        console.log(`[Stock Adventure] Level completed! Moving to level ${newLevel}:`, newProgress)
        setProgress(newProgress)
      ...
      console.log(`[Stock Adventure] Moving to question ${newQuestion}:`, newProgress)
      setProgress(newProgress)
  ...
  const handleShowHint = () => {
    console.log("[Stock Adventure] Hint requested by player")
    setShowHint(true)
  }

  const resetGame = () => {
    console.log("[Stock Adventure] Resetting game to initial state")
    const resetProgress = {
      currentLevel: 0,
      currentQuestion: 0,
      completedLevels: [],
    ...
    console.log("[Stock Adventure] Game reset complete")
  }
